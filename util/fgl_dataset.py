...
from util.WikiPages_util import *
...
class FGLDataset(Dataset):
    ...
    def load_global_graph(self):
        if self.name in ["Cora", "CiteSeer", "PubMed"]:
            ...
            self.global_dataset = WikiCS(root=self.raw_dir)
        elif self.name in ['Chameleon','Squirrel']:
            self.global_dataset = WikiPages(root=self.raw_dir, name=self.name)
        elif self.name in ['Amazon-ratings', 'Roman-empire','Questions']:
            from torch_geometric.datasets import HeterophilousGraphDataset
            self.global_dataset = HeterophilousGraphDataset(root=self.raw_dir, name=self.name)
        elif self.name in ['Actor']:
            from torch_geometric.datasets import Actor
            self.global_dataset = Actor(root=self.raw_dir)
